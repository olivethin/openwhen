...
export default function CapsuleListScreen({ navigation }) {
  // 1) Load fonts (do NOT early-return)
  const [fontsLoaded] = useFonts({ Poppins_700Bold });

  // 2) Other hooks (always run, every render, in same order)
    ...
    >
      <SafeAreaView style={{ flex: 1 }}>
        ...
        <View style={styles.header}>
          <Text style={[styles.headerTitle, fontsLoaded && { fontFamily: "Poppins_700Bold" }]}>
            Your Capsules
          </Text>
        </View>

        {/* Big picture absolutely positioned (header won't grow) */}
        ...
        {/* If fonts aren't loaded yet, show a small overlay loader (no early return) */}
        {!fontsLoaded && (
...
const styles = StyleSheet.create({
  ...
  headerTitle: {
    fontSize: 26,
    // fontFamily is set conditionally when loaded to avoid warnings
    color: "#0f172a",
    letterSpacing: 0.3,
  },
